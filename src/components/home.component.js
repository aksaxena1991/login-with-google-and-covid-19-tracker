      ...
      <MDBRow style={{ width: "100%", marginTop: "1%" }}>
        ...
        <MDBCol size="6">
          <div className="text-center">
            <MDBCard narrow>
              <MDBCardImage className='view view-cascade gradient-card-header purple-gradient' cascade tag='div'>
                <MDBCardHeader tag='h4' className='text-center'>World Map</MDBCardHeader>
              </MDBCardImage>
              <MDBCardBody cascade className='text-center'>
              <Map className = "map" center = { [this.state.currentLocation.lat, this.state.currentLocation.lng] } zoom = { this.state.zoom }>
                <TileLayer attribution='&amp;copy <a href="http://osm.org/copyright">OpenStreetMap</a> contributors'
                  url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"/>
                <Marker position = { [this.state.currentLocation.lat, this.state.currentLocation.lng] } icon = { myIcon}>
                  <Popup>
                    <div className="text-center"> <MDBBox tag='p'>{this.state.markerPopup.state}</MDBBox> </div>
                  </Popup>
                </Marker>
              </Map>
              </MDBCardBody>
